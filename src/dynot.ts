...
import { basename, dirname, extname, join } from 'nyxpath'
import destr from 'nyxjson'
import escapeStringRegexp from 'escape-string-regexp'
import createRequire from 'requireflow'
import { lt } from 'semver'
import { normalizeAliases, resolveAlias } from 'nyxpath/utils'
import { addHook } from 'pirates'
import objectHash from 'object-hash'
import { hasESMSyntax, interopDefault, resolvePathSync } from 'esmnxuts'
import {
   detectLegacySyntax,
   getCacheDir,
   isDir,
...
import type { DYNOTOptions, TransformOptions } from './types'

const _EnvDebug = destr(process.env.DYNOT_DEBUG)
const _EnvCache = destr(process.env.DYNOT_CACHE)
const _EnvESMResolve = destr(process.env.DYNOT_ESM_RESOLVE)
const _EnvRequireCache = destr(process.env.DYNOT_REQUIRE_CACHE)
const _EnvSourceMaps = destr(process.env.DYNOT_SOURCE_MAPS)
const _EnvAlias = destr(process.env.DYNOT_ALIAS)
const _EnvTransform = destr(process.env.DYNOT_TRANSFORM_MODULES)
const _EnvNative = destr(process.env.DYNOT_NATIVE_MODULES)

const isWindows = platform() === 'win32'

const defaults: DYNOTOptions = {
   debug: _EnvDebug,
...
export interface DYNOT extends Require {
...
export default function createDYNOT(
   _filename: string,
   opts: DYNOTOptions = {},
   ...
   const alias = (opts.alias && Object.keys(opts.alias).length > 0)
      ? normalizeAliases(opts.alias || {})
      : null

   // List of modules to force transform or native
   const nativeModules = ['typescript', 'dynot', ...(opts.nativeModules || [])]
   ...
   function getCache(
      ...
      if (!result.includes('__DYNOT_ERROR__'))
   ...
   function dynot(id: string) {
      ...
      mod.require = createDYNOT(filename, opts, mod, requiredModules || {})
      ...
      if (mod.exports && mod.exports.__DYNOT_ERROR__) {
         const { filename, line, column, code, message }
        = mod.exports.__DYNOT_ERROR__
         const loc = `${filename}:${line}:${column}`
         const err = new Error(`${code}: ${message} \n ${loc}`)
         Error.captureStackTrace(err, dynot)
         opts.onError!(err)
      }
   ...
   function register() {
      return addHook(
         (source: string, filename: string) =>
            dynot.transform({ source, filename, ts: !!/\.[cm]?ts$/.test(filename) }),
         { exts: opts.extensions },
      )
   }

   dynot.resolve = _resolve
   dynot.cache = opts.requireCache ? nativeRequire.cache : {}
   dynot.extensions = nativeRequire.extensions
   dynot.main = nativeRequire.main
   dynot.transform = transform
   dynot.register = register
